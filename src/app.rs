...
impl<'a> App<'a> {
    ...
    pub fn handle_text_edit_input(&mut self, key: crossterm::event::KeyEvent) {
        self.input.handle_event(&Event::Key(key));
    }

    pub fn on_enter(&mut self) {
        if self.tabs.index == 0 {
            let import_string = self.input.value();
            let mut debug_string = String::new();
            let _ = Profile::import_mods(import_string, &mut debug_string);
            self.input.reset();
        }
    }
}
